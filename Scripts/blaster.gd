...
@export var attack_gap_duration: float = 2.0
@export var single_attack: bool = true
@export var knockback_vector: Vector2 = Vector2(400, -300)
@export var knockback_duration: float = 0.8
var gravity = ProjectSettings.get_setting("physics/2d/default_gravity")
var blast_scene: PackedScene = preload("res://Instances/blast.tscn")
...
func shoot() -> void:
	var blast: Area2D = blast_scene.instantiate()
	
	is_shooting = true
	blast.damage = damage
	blast.second_damage = damage / 2.0
	blast.single_attack = single_attack
	if single_attack:
		blast.target_uid = shooting_range_opponents_uid[0]
	battlefield.call_deferred("add_child", blast)
